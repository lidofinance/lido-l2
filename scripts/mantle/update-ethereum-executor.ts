...
async function main() {
  ...
  const [, mntRunner] = ethMntNetwork.getSigners(env.privateKey(), {
    forking: isForking,
  });
  ...
  console.log(`  · L2 tx runner: ${mntRunner.address}`);
  console.log(`  · L2 tx runner ETH balance: ${await mntRunner.getBalance()}`);
  ...
  const { callvalue, calldata } = await mantle
  ...
  const createVoteTx = await oldEthExecutorLidoDAO.createVote(
    l1LDOHolder,
    "Update ethereumGovernanceExecutor on Mantle Governance Bridge Executor",
    {
