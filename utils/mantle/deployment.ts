...
interface MntL1DeployScriptParams {
  deployer: Wallet;
  admins: { proxy: string; bridge: string };
}

interface MntL2DeployScriptParams extends MntL1DeployScriptParams {
  l2Token?: { name?: string; symbol?: string };
}

interface MntDeploymentOptions extends CommonOptions {
  logger?: Logger;
  overrides?: Overrides;
}
...
export default function deployment(
  ...
  options: MntDeploymentOptions = {}
) {
  const mntAddresses = addresses(networkName, options);
  return {
    async erc20TokenBridgeDeployScript(
      l1Token: string,
      l1Params: MntL1DeployScriptParams,
      l2Params: MntL2DeployScriptParams
    ) {
      const [
