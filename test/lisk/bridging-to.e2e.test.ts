...
scenario("Lisk :: Bridging via depositTo/withdrawTo E2E test", ctxFactory)
  .step(
...
async function ctxFactory() {
  const networkName = env.network("TESTING_LISK_NETWORK", "goerli");
  const testingSetup = await lisk.testing(networkName).getE2ETestSetup();
