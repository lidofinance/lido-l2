...
scenario("Lisk :: Bridging integration test", ctxFactory)
  .after(async (ctx) => {
...
async function ctxFactory() {
  const networkName = env.network("TESTING_LISK_NETWORK", "mainnet");
  ...
  const {
    ...
    ...contracts
  } = await lisk.testing(networkName).getIntegrationTestSetup();

  const l1Snapshot = await l1Provider.send("evm_snapshot", []);
  const l2Snapshot = await l2Provider.send("evm_snapshot", []);

  await lisk.testing(networkName).stubL1CrossChainMessengerContract();
